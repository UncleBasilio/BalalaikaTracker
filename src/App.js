...
function Stats({ stats, period, title }) {
  if (!stats) {
    return null;
  }
...
function App() {
  ...
  const stopButton = (
    <Button variant="success" onClick={stopTimer}>
      Стоп
    </Button>
  );
  ...
  return (
    <div className="container">
      <div className="d-flex flex-column align-items-center justify-content-center">
        ...
        <Stats period={getTodayPeriod()} stats={stats} title="Сегодня" />
        <Stats
          period={getLast7DaysPeriod()}
          stats={stats}
          title="Последние 7 дней"
        />
        <Stats period={getThisWeekPeriod()} stats={stats} title="Неделя" />
        <Stats period={getThisMonthPeriod()} stats={stats} title="Месяц" />
        <Stats period={getThisYearPeriod()} stats={stats} title="Год" />
      </div>
    </div>
  );
}
